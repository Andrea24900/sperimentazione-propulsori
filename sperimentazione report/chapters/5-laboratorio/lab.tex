...
\begin{figure}
	\centering
	\includegraphics[width=0.7\linewidth]{"chapters/5-laboratorio/tabellaqmcd"}
	\caption{Dipendenza di \gls{symb:cd} da $q_M$}
	\label{fig:tabellaqmcd}
\end{figure}
\begin{figure}
	\centering
	\includegraphics[width=0.7\linewidth]{"chapters/5-laboratorio/tabelladeltapcd"}
...
\begin{figure}
	\centering
	\includegraphics[width=0.7\linewidth]{"chapters/5-laboratorio/tabellaTcd"}
...
Al fine di individuare una stima attendibile di \gls{symb:cd} si sceglie di proporre due valori medi di due gruppi di misure: ad essi è associato l'errore statistico con intervallo di confidenza al 95\,\%. I risultati sono riportati in Tab.\ref{tab:stimacd}.
\begin{table}[H]
	\centering
	\begin{tabular}{c|c|c}
		\toprule
		\toprule
		& \textbf{Serie 1} & \textbf{Serie 2}  \\
		\midrule
		\midrule
		Misure impiegate & da 3 a 12 & da 8 a 12 \\
		\midrule
		Numero misure & 10 & 5 \\
		\midrule
		$\overline{C_D}$ & 0.906 & 0.873 \\
		\midrule
		\gls{symb:errstat} & $\pm$ 0.01 & $\pm$ 0.0001 \\
		
		
		\bottomrule
		\bottomrule
	\end{tabular}
\caption{Stima di \gls{symb:cd}}
\label{tab:stimacd}
\end{table}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\linewidth]{chapters/5-laboratorio/ma3}
...
Mediante la prova sperimentale si individua una stima di \gls{symb:cd} dell'ugello, parte del banco prova. In prima analisi è possibile stimare l'errore associato alla misura. Il valore stimato di \gls{symb:cd} è compatibile con la fisica del problema e ritenuto accettabile. Dall'osservazione di \gls{symb:cd} al variare di \gls{symb:re} e \gls{symb:ma} emerge che le ipotesi utilizzate per la soluzione sono valide. 